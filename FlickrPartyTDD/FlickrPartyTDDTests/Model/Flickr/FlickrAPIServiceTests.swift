...
class FlickrAPIServiceTests: XCTestCase {
    ...
    func testSearch_MakesRequestFromTag() {
        ...
        guard let params = urlComponents?.queryItems
            else { XCTFail("URL has no query parameters"); return }
        ...
        XCTAssertEqual(tagQueryItem?.value, "party",
                       "Query param should match search() method argument")

        XCTAssertEqual(pageNumberQueryItem?.value, "1",
                       "Query param should match search() method argument")
    }
    ...
    func testSearch_OnCompletionReturnsExpectedData() {
        ...
        let urlResponse =
            NSHTTPURLResponse(URL: NSURL(string: "http://example.com")!,
        ...
        sut.search("party", pageNumber: 1) { (result, error) in
            ...
            guard let data = result?.dataUsingEncoding(
                NSUTF8StringEncoding) else { XCTFail(); return }

            guard let jsonObj = try? NSJSONSerialization.JSONObjectWithData(
                data, options: NSJSONReadingOptions.init(rawValue: 0))
                as? [String:AnyObject] else {
                    XCTFail()
                    return
            }

            guard let jsonDict = jsonObj else { XCTFail(); return }

            XCTAssertEqual(jsonDict["result"] as? String, "success")
        }
    ...
    func testSearch_NilDataReturnsInvalidResponseError() {
        ...
        sut.search("party", pageNumber: 1) { (result, error) in
            XCTAssertNotNil(error,
                            "API service must return error when invalid data recieved")
            XCTAssertEqual(error, ServiceError.InvalidResponse,
                           "Error must be InvalidResponse")
        }
    }
