...
class GalleryDataProvider: NSObject, PhotoDataProvider, ReachabilityListener {
    ...
    func registerCellIdentifiers(collectionView: UICollectionView) {
        ...
        collectionView.registerClass(
            ...
            forCellWithReuseIdentifier: LoadMoreCollectionViewCell.reuseIdentifier)

        collectionView.registerClass(
            RefreshCollectionViewCell.self,
            forCellWithReuseIdentifier: RefreshCollectionViewCell.reuseIdentifier)
    ...
    func collectionView(collectionView: UICollectionView,
        ...
        guard let photoManager = photoManager else { return 0 }
    ...
    func collectionView(collectionView: UICollectionView,
        ...
        if indexPath.item == photoCount - 12 {
            if isNetworkReachable { photoManager?.loadMore() }
        }
        ...
        guard indexPath.item < photoCount else {
            ...
            if isNetworkReachable {
                let cell = collectionView.dequeueReusableCellWithReuseIdentifier(
                    LoadMoreCollectionViewCell.reuseIdentifier,
                    forIndexPath: indexPath)
                return cell
            } else {
                let cell = collectionView.dequeueReusableCellWithReuseIdentifier(
                    RefreshCollectionViewCell.reuseIdentifier,
                    forIndexPath: indexPath)
                return cell
            }
        }
        ...
        identifier, forIndexPath: indexPath) as? GalleryCollectionViewCell else {
            fatalError()
        }
