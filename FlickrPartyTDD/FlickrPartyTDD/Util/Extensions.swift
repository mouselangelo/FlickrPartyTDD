...
extension UIView {
    /**
     Adds a subview and adds the autolayout constraints to pin all edges to the parent
     (current) view
    ...
    /**
     Adds a subview and adds the autolayout constraints to center this view to the parent
     (current) view
     ...
     */
    func addSubViewCentered(target: UIView,
        ...
        let horizontalCenter =
            NSLayoutConstraint(item: self, attribute: .CenterX, relatedBy: .Equal,
                               toItem: target, attribute: .CenterX,
                               multiplier: 1, constant: horizontalOffset)

        let verticalCenter =
            NSLayoutConstraint(item: self, attribute: .CenterY, relatedBy: .Equal,
                               toItem: target, attribute: .CenterY,
                               multiplier: 1, constant: verticalOffset)
...
extension UIViewController {
    /**
     ...
     Two `UIAlertActions` - *Cancel* and *Retry* are also displayed.
     The `retryCallback` closure will be called (if provided) when user chooses *Retry*
    ...
    */
    func showNoANetworAlert(retryCallback: (() -> Void)?) -> UIAlertController {
        ...
        alertVC.addAction(UIAlertAction(title: "Retry",
