...
class FlickrResponseParser {
    ...
    func parse(json: String, handler: ParserHandler) {
        ...
        guard let data =
            ...
            else { invalidJSON(handler); return }
        ...
        guard let result =
            jsonResponse[JSONKeys.photos] as? [String:AnyObject]
            else { invalidJSON(handler); return }
        ...
        guard let photoArray =
            result[JSONKeys.photo] as? [AnyObject]
            else { invalidJSON(handler); return }
    ...
    private func parsePhoto(photoDict: AnyObject) -> Photo? {
        ...
        guard let farmId =
            photoDict[JSONKeys.photoFarmId] as? Int,
            serverId = photoDict[JSONKeys.photoServer] as? String,
            photoId = photoDict[JSONKeys.photoId] as? String,
            secret = photoDict[JSONKeys.photoSecret] as? String
            else {return nil}
        ...
        if let photoURL =
            NSURL(string: photoURLString),
            thumbnailURL = NSURL(string: thumbnailURLString) {

            let photo = PhotoItem(url: photoURL,
