...
class FlickrAPIService {
    ...
    func search(tag: String, pageNumber:Int, completion: Handler) {
        ...
        guard let url = buildURL(config.searchMethod, params: params) else { fatalError("Unable to build URL") }
        ...
        sendRequest(url) {  (data, response, error) in
            ...
            guard let response = response as? NSHTTPURLResponse else { fatalError("Not HTTP response?")}
    ...
    private func buildURL(method:String, params:[String : String]?) -> NSURL? {
        ...
        guard let urlComponents = NSURLComponents(string: config.apiEndpoint) else { fatalError("Invalid Endpoint URL") }
